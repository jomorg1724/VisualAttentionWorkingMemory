...
## Addendum 2026-09-17T14:30-07:00: local re-run and analyses

The kda arm re-run locally (seed 1, `runs/local_kda_program_20260917/`) reproduced the pod result: test BA 1.000 at D0/4/12/24, with checkpoints kept. `analysis/kda_probe.py` and `analysis/psychometric.py` were run on its final model; results are written into `SecondPass/KDA_paper/KDA_paper.md` Sections 6.7 and 7.4 and stored under `runs/local_kda_program_20260917/analysis/`. Headline: state reset before the probe gives chance at every delay; write gate 0.77 on samples and 0.52 on blanks, retention 0.94 and 0.96; clamping blank writes off or retention to one changes nothing; the probe's implicit attention on blanks is 0.002-0.004 at D24; a linear read of the 25x25 state gives the sample angle at R² 0.99 on every frame through D24; psychometric threshold 2.2-2.9 degrees at all delays (trained on 15-45); BA 1.000 at delay 48 (trained to 24); no distractor effect; the only fragility is cue contrast and glyph position (0.54 at quarter contrast, 0.85 at 4 px jitter).
