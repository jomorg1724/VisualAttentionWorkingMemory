"""Versioned ordered-task worker. No implicit launch, budget renewal or task expansion.
...
def worker(job):
    ...
    if job.get('checkpoint'):
        ...
        if cp['version']!='pav_ordered_tasks_v1' or cp['config']!=cfg or cp['source_hashes']!=job['source_hashes']:
    ...
    def save():
        ...
        if not path.exists():
            state=dict(version='pav_ordered_tasks_v1',config=cfg,source_hashes=job['source_hashes'],step=step,
    ...
    if kind in ('train','profile'):
        ...
        if not job.get('checkpoint'):raise ValueError('Evaluation requires a saved checkpoint')
        ...
        result.update(status='completed',split=job['split'],step=step,evaluated_pairs=len(rows),
            worker_seconds=time.monotonic()-started,predictions=str(out/'predictions.jsonl'),checkpoint=job['checkpoint'])
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: train_multitask.py AUTHORIZED_JOB.json')
