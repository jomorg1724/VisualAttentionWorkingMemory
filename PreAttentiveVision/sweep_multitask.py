"""Single-GPU sequential seven-task comparison within the remaining allowance."""
import os
import sys
import time
import json
...
REMAINING_SECONDS=3057.776230096817
...
def run(root):
    ...
    names=['models.py','decoder.py','decoder_multitask.py','train.py','train_multitask.py',
           'evaluate.py','evaluate_multitask.py','neuroscience_stimuli.py','natural_stimuli.py','sweep_multitask.py']
    ...
    ledger=dict(started_unix=now,deadline_unix=deadline,hard_seconds=REMAINING_SECONDS,
        previous_compute_charge_seconds=542.223769903183,total_authorized_seconds=3600,
    ...
    aggregate=dict(status='profiling',run_root=str(root),config=dict(models=MODEL_NAMES,task_classes=TASK_CLASSES,
        batch_size=32,seeds=[20271],budget_seconds=REMAINING_SECONDS),profiles={},runs=[])
    def publish():
        write(root/'aggregate.json',aggregate);write(HERE/'results_multitask.json',aggregate)
    ...
    def launch(kind,cfg,out,**kwargs):
        ...
        job=dict(kind=kind,config=cfg,out=str(out),deadline=deadline,result=str(result),source_hashes=hashes,**kwargs)
        write(job_path,job);log_path=root/f'worker_{number:03d}.log';tick=time.time()
        with log_path.open('w') as log:
            process=subprocess.Popen([sys.executable,'-B',str(HERE/'train_multitask.py'),str(job_path)],
    ...
    try:
        for name in MODEL_NAMES:
            cfg=dict(base,model=name,seed=997,purpose='separate_accounted_profile')
            profile,wall=launch('profile',cfg,root/'profiles'/name,target=21)
        ...
        # Three balanced validation looks, one final test all five, and startup/checkpoint/bootstrap reserve.
        val_n=224;test_n=448;eval_batches=3*7*math.ceil(val_n/32)+7*math.ceil(test_n/32)
        reserve=eval_batches*evalcost+250
        remaining=deadline-time.time()
        updates=next((u for u in [2016,1512,1008,756,504,252,126] if u*traincost+reserve<remaining*.92),None)
        if updates is None:raise RuntimeError('Remaining allowance cannot fit balanced all-five training plus evaluation')
        targets=[updates//3,2*updates//3,updates]
        aggregate['config'].update(updates=updates,targets=targets,training_pairs_per_model=updates*32,
            training_pairs_per_task_per_model=updates*32//7,val_pairs_per_task=val_n,test_pairs_per_task=test_n,
        ...
        for name in MODEL_NAMES:
            aggregate['runs'].append(dict(model=name,seed=20271,status='pending',step=0,training_seconds=0.,val_curve=[],test=None))
        publish()
        for target in targets:
            for record in aggregate['runs']:
                name=record['model'];out=root/f'{name}_seed20271';cfg=dict(base,model=name,seed=20271,purpose='seven_task_production')
                ...
                val,_=launch('eval',cfg,out/f'eval_val_{target:06d}',checkpoint=record['checkpoint'],split='val',eval_seed=840001,n_per_task=val_n,resamples=0)
        ...
        for record in aggregate['runs']:
            out=root/f"{record['model']}_seed20271";cfg=dict(base,model=record['model'],seed=20271,purpose='seven_task_production')
            test,_=launch('eval',cfg,out/'eval_test',checkpoint=record['best_checkpoint'],split='test',eval_seed=940001,n_per_task=test_n,resamples=1000)
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: sweep_multitask.py NEW_RUN_ROOT')
    run(sys.argv[1])
