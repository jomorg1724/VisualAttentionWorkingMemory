"""Two-arm task-allocation comparison within the last remaining allowance."""
import os
import sys
import time
import json
...
PARENT_ROOT=HERE/'runs'/'hybrids_20260912_152427'
PARENT_CP=PARENT_ROOT/'convnext_se_residual_seed20271'/'checkpoint_001512.pt'


def run(root):
    MODEL_NAMES=ARMS
    from PreAttentiveVision.neuroscience_stimuli import TASK_CLASSES
    prior_receipt=read(PARENT_ROOT/'analysis_completion_receipt.json')
    spent=[prior_receipt['total_charged_seconds']]
    remaining_seconds=prior_receipt['unused_seconds_of_original_3600']
    ...
    names=['models.py','decoder.py','decoder_multitask.py','train.py','train_multitask.py',
           'evaluate.py','evaluate_multitask.py','neuroscience_stimuli.py','natural_stimuli.py','train_hybrids.py','hybrid_transfer.py','hybrid_models.py','allocation_sampler.py','train_allocation.py','sweep_allocation.py']
    ...
    base=dict(batch_size=32,lr=.001,weight_decay=.0001,grad_clip=5.,fp32=True,amp=False,
        ...
        training_recipe='One32-pair task batch/update; uniform7cycle versus interleaved contour/other12cycle; task-local streams270001+100003*(task_index+1)',sampler_protocol='task_local_allocation_v1',
    ...
    aggregate=dict(status='profiling',run_root=str(root),config=dict(models=MODEL_NAMES,task_classes=TASK_CLASSES,
        batch_size=32,seeds=[20271],budget_seconds=remaining_seconds,parent_checkpoint=str(PARENT_CP),parent_sha256=parent_hash,parent_step=1512,engineering_criterion='At least.95balanced accuracy on EVERYtask; ambitious engineering target, not guaranteed horizon'),profiles={},runs=[])
    def publish():
        write(root/'aggregate.json',aggregate);write(HERE/'results_allocation.json',aggregate)
    ...
    def launch(kind,cfg,out,**kwargs):
        ...
        with log_path.open('w') as log:
            process=subprocess.Popen([sys.executable,'-B',str(HERE/'train_allocation.py'),str(job_path)],
    ...
    try:
        for name in MODEL_NAMES:
            ...
            profile,wall=launch('profile',cfg,root/'profiles'/name,target=1596)
            profile['full_profile_wall_seconds']=wall;aggregate['profiles'][name]=profile;publish()
        traincost=sum(p['train_step_mean']*1.35 for p in aggregate['profiles'].values())
        evalcost=sum(p['eval_batch_mean']*1.3 for p in aggregate['profiles'].values())
        # Three validation looks, final tests for both arms, and analysis/overhead reserve.
        val_n=224;test_n=448;eval_batches=3*7*math.ceil(val_n/32)+7*math.ceil(test_n/32)
        reserve=eval_batches*evalcost+150
        remaining=deadline-time.time()
        updates=next((u for u in list(range(1008,251,-84)) if u*traincost+reserve<remaining*.94),None)
        if updates is None:raise RuntimeError('Remaining allowance cannot fit balanced two-arm training plus evaluation')
        targets=[1512+max(84,round(updates/3/84)*84),1512+round(2*updates/3/84)*84,1512+updates]
        from PreAttentiveVision.allocation_sampler import exposure_counts,SCHEDULES,TaskLocalStreams
        assert len(set(targets))==3 and targets==sorted(targets)
        aggregate['config'].update(additional_updates=updates,final_step=1512+updates,targets=targets,additional_training_pairs_per_arm=updates*32,
            terminal_additional_pairs_per_task={arm:{task:n*32 for task,n in exposure_counts(updates,arm).items()} for arm in ARMS},schedules=SCHEDULES,training_derived_seeds=TaskLocalStreams(270001,'train').derived_seeds,val_pairs_per_task=val_n,test_pairs_per_task=test_n,
        ...
        for name in MODEL_NAMES:
            aggregate['runs'].append(dict(model=name,seed=20271,status='pending',step=1512,training_seconds=0.,val_curve=[],test=None))
        publish()
        for target in targets:
            for record in aggregate['runs']:
                name=record['model'];out=root/f'{name}_seed20271';cfg=dict(base,model=name,seed=20271,purpose='task_allocation_production')
                trained,wall=launch('train',cfg,out,target=target,checkpoint=record.get('checkpoint'))
                record.update(status='running',step=trained['step'],checkpoint=trained['checkpoint'],train_pairs=trained['fresh_pairs'],additional_pairs=trained['additional_pairs'],additional_updates=trained['additional_updates'],per_task_additional_updates=trained['per_task_additional_updates'])
                ...
                val,_=launch('eval',cfg,out/f'eval_val_{target:06d}',checkpoint=record['checkpoint'],split='val',eval_seed=870001,n_per_task=val_n,resamples=0)
                minimum=min(v['overall']['balanced_accuracy'] for v in val['tasks'].values())
                record['val_curve'].append(dict(step=target,min_task_ba=minimum,per_task_additional_updates=trained['per_task_additional_updates'],macro_ovr_auc=val['macro_ovr_auc'],mean_normalized_balanced_accuracy=val['mean_normalized_balanced_accuracy'],tasks=val['tasks']))
                if record.get('best_min_task_ba') is None or (minimum,val['macro_ovr_auc'])>(record['best_min_task_ba'],record['best_auc']):
                    record.update(best_min_task_ba=minimum,best_auc=val['macro_ovr_auc'],best_step=target,best_checkpoint=record['checkpoint'],selected_per_task_additional_updates=trained['per_task_additional_updates'])
                publish()
        aggregate['status']='final_test';publish()
        for record in aggregate['runs']:
            out=root/f"{record['model']}_seed20271";cfg=dict(base,model=record['model'],seed=20271,purpose='task_allocation_production')
            test,_=launch('eval',cfg,out/'eval_test',checkpoint=record['best_checkpoint'],split='test',eval_seed=970001,n_per_task=test_n,resamples=1000)
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: sweep_allocation.py NEW_RUN_ROOT')
    run(sys.argv[1])
