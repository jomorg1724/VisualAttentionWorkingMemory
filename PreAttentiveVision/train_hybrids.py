"""Explicit parent-derived hybrid worker. No implicit launch or budget renewal.
...
def worker(job):
    ...
    from PreAttentiveVision.hybrid_transfer import transfer_parent
    ...
    encoder=build_encoder('convnext_grn') if cfg['model']=='convnext_grn' else build_hybrid(cfg['model'])
    ...
    stream=TaskStream(seed+100000,split='train');step=0
    if job.get('checkpoint'):
        cp=torch.load(job['checkpoint'],map_location='cpu',weights_only=False)
        if cp['version']!='pav_hybrid_continuation_v1' or cp['config']!=cfg or cp['source_hashes']!=job['source_hashes']:
        ...
        transfer=transfer_parent(model,optimizer,parent,cfg['model'])
        stream.load_state_dict(parent['stream']);step=parent['step']
    ...
    def save():
        ...
        if not path.exists():
            state=dict(version='pav_hybrid_continuation_v1',config=cfg,source_hashes=job['source_hashes'],step=step,
                lineage=dict(parent_checkpoint=job['parent_checkpoint'],parent_sha256=job['parent_sha256'],parent_step=756),
    ...
    if kind in ('train','profile'):
        ...
        with (out/'metrics.csv').open('a',newline='') as f:
            ...
            while step<job['target']:
                ...
                task=task_names[step%len(task_names)];tick=time.monotonic()
        ...
        result=dict(status='completed' if step==job['target'] else 'budget_stopped',step=step,
            checkpoint=save(),fresh_pairs=step*cfg['batch_size'],additional_pairs=(step-756)*cfg['batch_size'],additional_updates=step-756,worker_seconds=time.monotonic()-started,
            per_task_updates={task:step//len(task_names)+int(j<step%len(task_names)) for j,task in enumerate(task_names)})
        if kind=='profile':
            ...
            result.update(train_step_median=float(np.median(timings[8:] or timings)),
        ...
        model.eval();evaluation=TaskStream(job['eval_seed'],split=job['split']);rows=[]
        for task in task_names:
...
if __name__=='__main__':
    if len(sys.argv)!=2:raise SystemExit('Usage: train_hybrids.py AUTHORIZED_JOB.json')
