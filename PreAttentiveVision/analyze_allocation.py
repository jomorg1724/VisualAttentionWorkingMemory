"""Paired CPU analysis of the predeclared task-allocation comparison; no inference."""
import os
...
def main(root):
    ...
    pairs=[('contour_focus','uniform')]
    comparisons=[]
    for candidate,reference in pairs:
        tasks={}
        specs=[(t,None) for t in aggregate['config']['task_classes']]+[('contour',d) for d in sorted({r['difficulty'] for r in rows[candidate] if r['task']=='contour'})]
        for task,difficulty in specs:
            a=[r for r in rows[candidate] if r['task']==task and (difficulty is None or r['difficulty']==difficulty)];b=[r for r in rows[reference] if r['task']==task and (difficulty is None or r['difficulty']==difficulty)]
            ...
            tasks[task if difficulty is None else task+'/'+difficulty]=dict(n=len(y),source_groups=len(groups),candidate_ba=float(ps[0]),reference_ba=float(qs[0]),
        ...
        strata={k.split('/',1)[1]:tasks.pop(k) for k in list(tasks) if k.startswith('contour/')}
        criterion=all(v['candidate_ba']>=.95 for v in tasks.values())
        comparisons.append(dict(model=candidate,reference=reference,tasks=tasks,
            engineering_criterion_met=criterion,reference_engineering_criterion_met=all(v['reference_ba']>=.95 for v in tasks.values()),contour_difficulty=strata))
    result=dict(status='completed',run_root=str(root.resolve()),comparisons=comparisons,
        criterion='At least95%balanced accuracy on every task; ambitious engineering point-estimate screen, not formal proof or guaranteed horizon',
    ...
    for path in (root/'paired_analysis.json',HERE/'results_allocation_analysis.json'):path.write_text(json.dumps(result,indent=2),encoding='utf-8')
