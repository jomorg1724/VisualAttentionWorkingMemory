...
def run(manifest_path):
    ...
    manifest=read(manifest_path);root=ROOT/'remote_results';root.mkdir(exist_ok=True)
    ...
    if budget_path.exists():
        ...
        agg=dict(status='profiling',platform=versions,runs={},parent=str(parent),parent_sha256=sha(parent),interpretation='Old task bias-removal comparison against saved equal-exposure biased control; new five-task training is a distinct experiment. Both initialize from attention8400.')
    ...
    publish()
    try:
        configs={a:recipe(a) for a in ('old_unbiased','spatial_unbiased')}
        if 'pinned_updates' not in agg:
            old=configs['old_unbiased'];new=configs['spatial_unbiased']
            old_names=['binding_D24','single_D24','motion_D24','binding_D0','single_D0','motion_D0']
            ...
            profiles={a:launch(a,configs[a],'profile','profile',profile_names=names) for a,names in [('old_unbiased',old_names),('spatial_unbiased',new_names)]};agg['profiles']=profiles
            op={r['name']:r for r in profiles['old_unbiased']['rows']};oldcost={}
            for n in old['cells']:
                family,d=n.split('_D');f=int(d)/24;oldcost[n]={k:(1-f)*op[family+'_D0'][k]+f*op[family+'_D24'][k] for k in ('seconds','eval_seconds')}
            oldtrain=1.25*4000*sum(oldcost[n]['seconds'] for n in old['cycle'])/80
            oldeval=1.4*(6*128/8*sum(v['eval_seconds'] for v in oldcost.values())+3*512/8*(sum(v['eval_seconds'] for v in oldcost.values())+sum(v['eval_seconds'] for n,v in oldcost.items() if n.startswith('binding'))))
            npf={r['name']:r for r in profiles['spatial_unbiased']['rows']};ncost={}
            for task,names in new['train_names'].items():
            ...
            if updates<200:raise RuntimeError('Even200 new-task updates cannot fit alongside fixed4000 old arm')
            agg['pinned_updates']=dict(old_unbiased=4000,spatial_unbiased=updates);agg['profile_costs']=dict(old_training=oldtrain,old_evaluation=oldeval,new_update=newstep,new_evaluation=neweval,spatial_target_reduced=updates<4000);publish()
