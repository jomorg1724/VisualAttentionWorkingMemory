"""Single explicit spatial-comparison worker under an absolute deadline."""
import os
...
def worker(job):
    import numpy as np
    import torch
    from WorkingMemory.SpatialComparison.model import migrate,VERSION,groups
    ...
    parent=torch.load(job['parent'],map_location='cpu');model,opt,lineage=migrate(parent,arm,cfg);del parent
    stream=SpatialStream(cfg['train_seed'],'train');step=0;counts=Counter();cell_counts=Counter();model.cuda()
    ...
    write(out/'initialization.json',dict(lineage=lineage,total_parameters=sum(p.numel() for p in model.parameters()),trainable_parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),state_entries=512 if arm=='dense_comparator' else 21632))
    def save():
    ...
    if kind=='profile':
        ...
        result=dict(status='completed',rows=rows,checkpoint=save(),peak_allocated_bytes=torch.cuda.max_memory_allocated(),peak_reserved_bytes=torch.cuda.max_memory_reserved(),parameters=sum(p.numel() for p in model.parameters()),state_entries=512 if arm=='dense_comparator' else 21632)
    elif kind=='train':
        ...
        result.update(status='completed',step=step,selection_mean_auc=float(np.mean(selected)),predictions=str(out/'predictions.jsonl'),checkpoint=job.get('checkpoint'))
