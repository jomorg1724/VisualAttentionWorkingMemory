...
def worker(job):
    ...
    if kind=='profile':
        ...
        result=summarize_spatial(rows);result.update(status='completed',step=step,predictions=str(out/'predictions.jsonl'),checkpoint=job.get('checkpoint') or job['parent']);write(out/'summary.json',result)
    elif kind=='eval':
