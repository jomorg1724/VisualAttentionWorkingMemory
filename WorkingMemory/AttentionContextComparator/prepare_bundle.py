...
from WorkingMemory.AttentionContextComparator.protocol import TOTAL_EPISODES, UPDATES
...
EXPERIMENT = (
    ...
    "WorkingMemory/AttentionContextComparator/check_model.py",
    "WorkingMemory/AttentionContextComparator/worker.py",
    "WorkingMemory/AttentionContextComparator/sweep.py",
)
EXCLUDE = (
...
def prepare(output, deadline):
    ...
    manifest = dict(
        experiment="attention_context_comparator_scratch_v1",
        ...
        authorized_arms=["attention_context_comparator_scratch"],
        architecture=(
            "Full scratch control except the learned Conv([R_old,H]) comparator "
            "is absent and C is merged attention-head context before W_O. "
            "U=W_O(C) and the terminal spatial priority readout are unchanged."
        ),
        entrypoint="WorkingMemory/AttentionContextComparator/sweep.py",
        command=(
            "python -B WorkingMemory/AttentionContextComparator/sweep.py "
