...
def main():
    ...
    if config.get("authorized_arms") != ["attention_context_comparator_scratch"]:
        raise RuntimeError("Watcher restricted to attention-context comparator arm")
    ...
    incremental = root / "incremental" / "attention_context_comparator_results"
...
m=r/'attention_context_comparator_scratch/training/metrics.csv'
lines=m.read_text().splitlines() if m.is_file() else []
idx=r/'attention_context_comparator_scratch/training/checkpoint_index.jsonl'
...
"""
    ...
    def transfer(relative, expected_sha):
        ...
        if not re.fullmatch(
            r"((profile|attention_context_comparator_scratch/(training|validation_[0-9]+|selected_test|terminal_test))/[A-Za-z0-9_.-]+|validation_[0-9]+_(job|result)\.json|validation_[0-9]+\.log)",
            relative,
        ):
...
"""
            ...
            if (
                resume_step
                and payload["total_rows"] > resume_step
                and (
                    not rows
                    or int(rows[-1]["step"]) <= resume_step
                    or not all(
                        __import__("math").isfinite(float(rows[-1][key]))
                        for key in ("loss", "accuracy", "gradient_norm", "seconds")
                    )
                )
            ):
                raise RuntimeError("Resume metric continuity/finite check failed")
            terminal = (
