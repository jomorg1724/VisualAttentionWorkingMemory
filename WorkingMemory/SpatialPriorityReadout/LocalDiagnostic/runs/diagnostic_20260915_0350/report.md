...
Mean target-region mass was 13.13% (95% CI 11.88%–14.56%); peak hit rate was 20.80% (95% CI 17.58%–24.12%). Uniform-map expectation is 5.33%. Target coordinates were used only for this post-test metric, not training. Maps are exposed in `priority_maps_test.npz` and `predictions_test.jsonl`.
...
Production computation took 414.6 seconds; total bounded wall time from the first smoke extraction through finalization was 738.5 seconds. See `results.json`, `run_manifest.json`, feature receipts and selected probe checkpoints for reproducible details.
