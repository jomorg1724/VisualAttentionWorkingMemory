...
PARENT=ROOT/'WorkingMemory/RecurrentComparison/Retention/runs/retention_20260913_084433/retention/checkpoint_014800.pt'
def protocol():
    cells={}
...
def run(root):
    ...
    cells,cycle=protocol();arms=('spatial_ei','dense_comparator')
    cfg=dict(cells=cells,cycle=cycle,batch_size=8,train_seed=32973001,scheduler_seed=33973001,model_seed=31973001,new_lr=.0003,parent_lr=.00003,adam_eps=1e-10,weight_decay=.0001,clip=1.,activation_checkpoint=True)
    names=list(read(PARENT.parents[1]/'budget.json')['source_hashes'])+['WorkingMemory/SpatialComparison/'+n for n in ('model.py','train.py','sweep.py','stimuli.py')]
    ...
    digest=sha(PARENT);assert digest=='b8ffa90c9e23b662ec3b16b3de2cb56ca9488b60d05a096aa5675f9081c588bc'
    ...
    agg=dict(status='profiling',run_root=str(root),config=dict(recipe=cfg,parent=str(PARENT),parent_sha256=digest,parent_step=14800,arms=arms,allowance_seconds=14400),profiles={},runs={})
    ...
    try:
        ...
        reserve=600+1.4*sum(evalcost.values());steps=next((n for n in range(4960,0,-80) if 1.3*n*sum(times.values())+reserve<deadline-time.time()-120),None)
        if steps is None:raise RuntimeError('No balanced equal exposure fits finite budget')
        targets=[round(steps*f) for f in (.25,.5,.75,1.)]
        agg['config'].update(updates_per_arm=steps,episodes_per_arm=steps*8,targets=targets,val_seed=34973001,test_seed=35973001,validation_n_per_cell=128,test_n_per_cell=512,selection='equal mean eight primary task-by-delay validation AUC; ties earlier',training_estimate_seconds=1.3*steps*sum(times.values()),evaluation_reporting_reserve_seconds=reserve,profile_updates_per_arm=6,profile_exposure_not_continued=True)
        ...
        for arm in arms:
            ...
            for target in targets:
                ...
                v=launch('eval',arm,root/f'{arm}_val_{target:06d}',checkpoint=cp,split='val',eval_seed=34973001,n=128,heldout_locations=False);record['validation'].append(v)
                ...
                publish()
            record['test']=launch('eval',arm,root/f'{arm}_test',checkpoint=record['selected_checkpoint'],split='test',eval_seed=35973001,n=512,heldout_locations=True);record['status']='completed';publish()
