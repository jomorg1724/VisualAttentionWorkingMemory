"""Bounded GPU worker for the scratch dual-attention priority arm."""
import copy
import csv
import json
import os
    ...
    def task_microbatch(name, diagnostic=False):
        ...
        if diagnostic:
            detail = dict(
                ...
                pre_attention_gradient_norm=float(
                    sum(
                        parameter.grad.square().sum()
                        for parameter in model.pre_attention.parameters()
                        if parameter.grad is not None
                    )
                    .sqrt()
                    .detach()
                ),
                post_attention_gradient_norm=float(
                    sum(
                        parameter.grad.square().sum()
                        for parameter in model.post_attention.parameters()
                        if parameter.grad is not None
                    )
                    .sqrt()
                    .detach()
