...
def main():
    ...
    if config.get("authorized_arms") != ["dual_attention_spatial_priority_scratch"]:
        raise RuntimeError("Watcher restricted to dual-attention arm")
    ...
    incremental = root / "incremental" / "dual_attention_results"
...
m=r/'dual_attention_spatial_priority_scratch/training/metrics.csv'
lines=m.read_text().splitlines() if m.is_file() else []
idx=r/'dual_attention_spatial_priority_scratch/training/checkpoint_index.jsonl'
...
"""
    ...
    def transfer(relative, expected_sha):
        ...
        if not re.fullmatch(
            r"((profile|dual_attention_spatial_priority_scratch/(training|validation_[0-9]+|selected_test|terminal_test))/[A-Za-z0-9_.-]+|validation_[0-9]+_(job|result)\.json|validation_[0-9]+\.log)",
            relative,
        ):
