...
OUT=Path(__file__).resolve().parent;SEED=16951001;START=json.loads((OUT/'supervisor.json').read_text())['start_epoch']
...
def main():
    ...
    train=dict(np.load(OUT/'train_features.npz')) if (OUT/'train_features.npz').exists() else extract(model,records['train'],'train')
    val=dict(np.load(OUT/'val_features.npz')) if (OUT/'val_features.npz').exists() else extract(model,records['val'],'val');fits={}
