...
def worker(job):
    import numpy as np
    import torch
    from WorkingMemory.SelectiveMaintenance.model import migrate,VERSION,groups
    ...
    write(out/'initialization.json',dict(lineage=lineage,total_parameters=sum(p.numel() for p in model.parameters()),trainable_parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),state_entries=21632+(64 if arm=='controller_feedback' else 0)))
    def save():
    ...
    def train_batch(name,record=False):
        ...
        if record:
            ...
            if 'controls' in diag:
                detail['early_controller_r_gradient_norm']=None if diag['controls'][0][0].grad is None else float(diag['controls'][0][0].grad.norm())
                detail['controller_gradient_norm']=float(sum(p.grad.square().sum() for p in model.controller.parameters() if p.grad is not None).sqrt())
    ...
    if kind=='profile':
        ...
        result=dict(status='completed',rows=rows,checkpoint=save(),peak_allocated_bytes=torch.cuda.max_memory_allocated(),peak_reserved_bytes=torch.cuda.max_memory_reserved(),parameters=sum(p.numel() for p in model.parameters()),state_entries=21632+(64 if arm=='controller_feedback' else 0))
    elif kind=='train':
        ...
        with (out/'predictions.jsonl').open('w') as f:
            for family in ('orientation_single','orientation_binding','motion_direction'):
                ...
                for mode in modes:
                    ...
                    cells=[(name,c) for name,c in cfg['cells'].items() if c['family']==family and (not job.get('feedback_off') or c['condition']['delay']==24)]
                    for offset in range(0,job['n'],cfg['batch_size']):
                        ...
                        for name,c in cells:
                            ...
                            off=None
                            if job.get('feedback_off'):
                                start_blank=10 if family=='motion_direction' else 3
                                off=set(range(start_blank,start_blank+c['condition']['delay']))
                            with torch.no_grad():prob=model(x.cuda(),FAMILY_HEAD[family],feedback_off_frames=off).softmax(1).cpu().tolist()
        ...
        result.update(status='completed',step=step,selection_mean_auc=float(np.mean(selected)),predictions=str(out/'predictions.jsonl'),checkpoint=job.get('checkpoint') or job['parent'],feedback_off=bool(job.get('feedback_off')))
