...
def worker(job):
    ...
    VERSION="ei_readout_only_continuation_v1"
    ...
    if parent['version']!='wm_focused_recurrent_v1' or parent['step']!=5000:raise RuntimeError('Expected strict EI5000 warm start')
    ...
    trainable_prefixes=('memory_output.','readout.heads.motion_direction.','readout.heads.orientation.')
    ...
    write(out/'initialization.json',dict(parent_step=5000,trainable_parameters=sum(p.numel() for p in model.parameters() if p.requires_grad),trainable_names=[n for n,p in model.named_parameters() if p.requires_grad],adam_state_transferred=True,sampler_state_transferred=True,all_upstream_eval=True))
    ...
    def save():
        ...
        if not path.exists():
            state=dict(version=VERSION,config=cfg,source_hashes=job['source_hashes'],step=step,
                lineage=dict(warm_start_version=VERSION,parent_checkpoint=job['parent_checkpoint'],parent_sha256=job['parent_sha256'],parent_step=5000,new_optimizer=False,expanded_protocol_only=True),
    ...
    def features(images):
        with torch.no_grad():
            ...
            for t in range(images.shape[1]):
                (field,r,z),state,_=model.step(images[:,t],state)
    ...
    def train_batch(family,condition,n):
        ...
        optimizer.zero_grad(set_to_none=True);sensory,r=features(x.cuda())
    ...
    if kind=='profile':
        ...
        step=5000+len(measurements);counts['episodes']=40000+len(measurements)*cfg['batch_size']
        ...
        # Without a new-protocol checkpoint, evaluate the untouched trained EI5000 parent.
