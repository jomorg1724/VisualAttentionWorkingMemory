...
def main():
    ...
    def sync_checkpoints():
        ...
        for row in json.loads(listing.stdout):
            name=row['path']
            if not re.fullmatch(r'(spatial_biased)/(profile|training)/checkpoint_[0-9]{6}\.pt',name):raise RuntimeError('Unexpected checkpoint path '+name)
    ...
    while time.time()<deadline+600:
        try:
            ...
            if check.returncode==0:
                ...
                pack=subprocess.run([*ssh,'cd '+remote+' && tar --exclude="*.pt" -czf biased_results.tar.gz biased_results && sha256sum biased_results.tar.gz'],capture_output=True,text=True,timeout=180,check=True);digest=pack.stdout.split()[0];archive=root/'biased_results.tar.gz'
                subprocess.run(['scp',*common,'-P',str(cfg['ssh_port']),'root@'+cfg['ssh_host']+':'+remote+'/biased_results.tar.gz',str(archive)],capture_output=True,text=True,timeout=600,check=True);assert sha(archive)==digest
                ...
                with tarfile.open(archive,'r:gz') as tar:
                    ...
                    tar.extractall(dest)
                fetched=dest/'biased_results';shutil.copytree(incremental,fetched,dirs_exist_ok=True);index=read(fetched/'artifact_index.json')
