...
Current execution correction: the user cancelled the five-task bias-removed run after 6,000 logged episodes, then explicitly requested a new run from intact attention 8400 with its original source/locality biases restored. The task definitions below are unchanged. The replacement is now training in `BiasedTraining`; its pinned 160,000-episode target shares the original cloud deadline. See [the journal decision](../../LabJournal/experiments/19-biased-spatial-battery.md).
...
Pinned validation matrix:12spatial task×delay cells,12recognition cells (four list lengths×three probe-repeat counts), and3Krauzlis baseline-length cells, totaling27conditions. Use64validation/256final episodes per non-Krauzlis condition and100validation/400final episodes per Krauzlis condition. The final renderer uses shuffled100-trial cycles with exactly57target,29foil and14catch events and50/50target-side totals per cycle. Event-to-side assignment reverses in the next cycle, balancing odd event counts across200trials. The100-example validation and400-example test therefore preserve the event mixture when starting at a cycle boundary; report actual counts too. This supersedes the earlier proposed unconstrained200-trial shuffle. Fresh validation/test seeds and actual counts remain in the launch configuration. These counts do not authorize exceeding the finite cap.
