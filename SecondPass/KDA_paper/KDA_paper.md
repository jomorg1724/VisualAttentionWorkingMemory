...
## 6.7 First results (local KDA re-run, seed 1, final delay model)

`kda_probe.py` was run on `runs/local_kda_program_20260917/kda_s1/delayC/terminal.pt` (test BA 1.000 at D0/4/12/24, the same as the pod run) with 128 trials per delay at the psychometric seed. Numbers are for the 25×25 scale unless stated; the two coarser scales behave the same way with weaker probes.

*Interventions.* Resetting the state before the probe gives BA 0.48, 0.52, 0.52 and 0.52 at D0, D4, D12 and D24: the memory is necessary at every delay, including D0, where the three-frame stack would in principle allow a direct comparison. Clamping $\beta=0$ on every blank, or $\boldsymbol\alpha=1$ on every blank, leaves BA at 1.000 at all delays: nothing written during blanks is used, and the learned retention is already as good as perfect retention.

*Gates.* Mean write strength $\beta$ at Gabor sites is 0.77 on sample frames, 0.52 on blanks (0.58 at D4, 0.52 at D24) and 0.75 at the probe; on background sites it is 0.54 throughout. Mean retention $\bar\alpha$ at Gabor sites is 0.94 on stimulus frames and 0.96 on blanks. So the network learned to write less and retain more when the input is gray, as Section 4.2 predicted, and it did so with no site-specific tuning at initialisation. The gates at cued and uncued Gabor sites are identical to two decimals: the memory writes every Gabor, and the cue is applied downstream by the readout, not by the write gate.

*Implicit attention.* The probe frame's attention weights $|w_{t,\tau}|$ over the past, averaged over the cued Gabor sites, peak on the second sample frame (0.22 at D0, 0.18 at D4, 0.16 at D12, 0.13 at D24) and on the probe itself (0.13), and are 0.002 to 0.004 on every blank frame at D24. The memory's read at the probe is a read of the samples, not of the recent gray.
...
## 7.4 First results (local KDA re-run, seed 1, final delay model)

`psychometric.py` was run on the same checkpoint as 6.7, 512 trials per point at the psychometric seed, balanced accuracy with 95% trial-bootstrap intervals (about ±0.02 at 0.9, ±0.04 at 0.75).

*Magnitude.* Trained only on 15°, 30° and 45°, the model reads much smaller rotations:
...
*Forgetting function.* At 15°, BA is 1.000 at delays 0, 1, 2, 4, 8, 12, 16, 24, 32 and 48. The exponential fit is degenerate (asymptote 1.000). Trained to 24, the memory holds at 48; the learned retention is not tuned to the training delays.

*Cue reliability (D4).* BA falls from 1.000 at the trained glyph to 0.947 and 0.850 with 2 px and 4 px position jitter, to 0.895 at half contrast, 0.537 at quarter contrast, and 0.500 at a tenth. Jitter and contrast loss compound (0.602 at half contrast with 4 px jitter). This is the one variable the observer is sensitive to.

*Distraction (D12).* BA is 0.994 with no uncued Gabors and 1.000 with one, two or three, whether they rotate as in the real task or all by the target's magnitude. The memory does not suffer interference from the other locations.

*Reading.* The three memory-side measurements (precision, persistence, distraction) all say the same thing as the probes in 6.7: the stored representation is precise, held without loss and site-local. The only fragility is the cue. Where the plain model's limit was retention, this model's limit is cue detection and the readout's use of it, which moves the next architectural question from memory to attention: how the cue selects a site and how robust that selection is to cue appearance. That question is the one the project's attention components were built for, and it can now be asked against a baseline whose memory is not the bottleneck.

## 7.5 Relation to human data
